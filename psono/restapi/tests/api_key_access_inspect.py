...
class CreateApiKeyAccessInspectTest(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        self.secret_data = json.dumps(
            {
                ...
                "secret_content_json": {"sub_secret_content": "some_variable_1"},
        ...
        self.api_key_secret_key_hex = nacl.encoding.HexEncoder.encode(
            api_key_secret_key
        )
        box = nacl.secret.SecretBox(
            self.api_key_secret_key_hex, encoder=nacl.encoding.HexEncoder
        )
    ...
    def test_post(self):
        ...
        self.assertEqual(
            response_data.get(
                "allow_insecure_access", not self.test_api_key_obj.allow_insecure_access
            ),
            self.test_api_key_obj.allow_insecure_access,
        )
        self.assertEqual(
            response_data.get(
                "restrict_to_secrets", not self.test_api_key_obj.restrict_to_secrets
            ),
            self.test_api_key_obj.restrict_to_secrets,
        )
        self.assertEqual(
            response_data.get("read", not self.test_api_key_obj.read),
            self.test_api_key_obj.read,
        )
        self.assertEqual(
            response_data.get("write", not self.test_api_key_obj.write),
            self.test_api_key_obj.write,
        )
        self.assertEqual(len(response_data.get("api_key_secrets", [])), 1)
        self.assertEqual(
            response_data["api_key_secrets"][0]["secret_id"],
            str(self.test_secret_obj.id),
        )
        self.assertEqual(
            response_data["api_key_secrets"][0]["write_date"],
            self.test_secret_obj.write_date.isoformat(),
        )
