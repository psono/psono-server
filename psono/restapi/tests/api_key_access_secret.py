...
class CreateApiAccessSecretKeyTest(APITestCaseExtended):
    ...
    def test_put_success(self):
        ...
        self.assertFalse(
            models.Secret.objects.filter(
                id=self.test_secret_obj.id, data=b"abc", data_nonce="def"
            ).exists()
        )
    ...
    def test_put_insecure_success(self):
        ...
        decrypted_db_secret_data = self.box.decrypt(
            nacl.encoding.HexEncoder.decode(db_secret.data),
            nacl.encoding.HexEncoder.decode(db_secret.data_nonce),
        )
    ...
    def test_read_secret_success_with_json_filter_and_nested_json(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertEqual(
            response.data, '{"sub_secret_content_nested": "some_variable_2"}'
        )
