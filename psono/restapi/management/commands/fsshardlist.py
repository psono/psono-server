...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if len(shards) == 0:
            print(
                "No shards found. You can create one with \"fsshardcreate TITLE 'Some description'\""
            )
        ...
        for shard in shards:
            ...
            if shard.links.count() == 0:
                print(
                    '    Cluster: Not linked to any cluster. You can create a link with "fsshardlink CLUSTER_ID '
                    + str(shard.id)
                    + '"'
                )
            ...
            for links in shard.links.all():
                print(
                    "    Cluster: "
                    + str(links.cluster.title)
                    + " (link_id: "
                    + str(links.id)
                    + ", cluster_id: "
                    + str(links.cluster_id)
                    + ", read: "
                    + str(links.read)
                    + ", write: "
                    + str(links.write)
