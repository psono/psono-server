...
class FileRepositoryDownloadView(GenericAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        serializer = FileRepositoryDownloadSerializer(
        ...
        if file_transfer.file_repository.type == "gcp_cloud_storage":
            base_url, query_params = gcs_construct_signed_download_url(
        ...
        if file_transfer.file_repository.type == "aws_s3":
            url = aws_construct_signed_download_url(
                data["aws_s3_bucket"],
                data["aws_s3_region"],
        ...
        if file_transfer.file_repository.type == "azure_blob":
            url = azure_blob_construct_signed_download_url(
        ...
        if file_transfer.file_repository.type == "backblaze":
            url = backblaze_construct_signed_download_url(
        ...
        if file_transfer.file_repository.type == "other_s3":
            url = s3_construct_signed_download_url(
        ...
        if file_transfer.file_repository.type == "do_spaces":
            url = do_construct_signed_download_url(
                data["do_space"],
                data["do_region"],
                data["do_key"],
                data["do_secret"],
        ...
        return Response(
            {"type": file_transfer.file_repository.type, "url": url},
