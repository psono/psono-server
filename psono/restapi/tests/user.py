...
class UserSearchTests(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        self.test_user_sauce = (
            "7d3fcc0a89f26e0f1277d0095620a10897c03becfb2f2b27684a55b8758a0020"
        )
        ...
        self.test_username2 = (
            "".join(random.choice(string.ascii_lowercase) for _ in range(10))
            + "some-partial-username@psono.pw"
        )
        ...
        self.test_user_sauce2 = (
            "5ca7f80a0e09fe13e02102f48a88c56b2c239056b86bb2694f7923d654651ab7"
        )
    ...
    def test_user_search_with_user_id(self):
        ...
        self.assertNotEqual(
            response.data.get("id", False), False, "id (user_id) not in response"
        )
        self.assertEqual(response.data.get("email", False), False, "email in response")
        self.assertNotEqual(
            response.data.get("username", False), False, "username not in response"
        )
        self.assertNotEqual(
            response.data.get("public_key", False), False, "public_key not in response"
        )
        self.assertEqual(
            response.data.get("id", False),
            self.test_user_obj2.id,
            "id does not match our test_user2_id",
        )
        self.assertEqual(
            response.data.get("username", False),
            self.test_user_obj2.username,
            "username from response does not match our username",
        )
        self.assertEqual(
            response.data.get("public_key", False),
            self.test_user_obj2.public_key,
            "public_key from response does not match our public_key",
        )

    @patch(
        "restapi.serializers.user_search.settings",
        ALLOW_USER_SEARCH_BY_USERNAME_PARTIAL=False,
        ALLOW_USER_SEARCH_BY_EMAIL=True,
        EMAIL_SECRET_SALT=settings.EMAIL_SECRET_SALT,
    )
    def test_user_search_with_user_username(self, mocked_settings_fnc):
        ...
        self.assertEqual(response.data.get("email", False), False, "email in response")
        self.assertNotEqual(
            response.data.get("username", False), False, "email not in response"
        )
        self.assertNotEqual(
            response.data.get("public_key", False), False, "public_key not in response"
        )
...
class UserActivateTokenTests(APITestCaseExtended):
    def setUp(self):
        ...
        self.test_user_sauce = (
            "7a1815d667e15d6310174e4b41c22fe618e18f1748091d07c4d79eef6ec02dd2"
        )
        self.test_user_obj = models.User.objects.create(
        ...
        session_crypto_box = Box(
            PrivateKey(user_session_private_key_hex, encoder=nacl.encoding.HexEncoder),
            PublicKey(
                self.request_data.get("session_public_key"),
                encoder=nacl.encoding.HexEncoder,
            ),
        )
        ...
        self.session_secret_key = session_crypto_box.decrypt(
            nacl.encoding.HexEncoder.decode(
                self.request_data.get("session_secret_key")
            ),
            nacl.encoding.HexEncoder.decode(
                self.request_data.get("session_secret_key_nonce")
            ),
        )

        # Second step is to decrypt the user_validator with the users private key and the servers session key
        user_crypto_box = Box(
            PrivateKey(self.test_private_key, encoder=nacl.encoding.HexEncoder),
            PublicKey(
                self.request_data.get("session_public_key"),
                encoder=nacl.encoding.HexEncoder,
            ),
        ...
        self.user_validator = user_crypto_box.decrypt(
            ...
            nacl.encoding.HexEncoder.decode(
                self.request_data.get("user_validator_nonce")
            ),
        )
    ...
    def test_post_authentication_activate_token(self):
        ...
        verification_crypto_box = nacl.secret.SecretBox(
            self.session_secret_key, encoder=nacl.encoding.HexEncoder
        )

        verification_nonce = nacl.utils.random(nacl.secret.SecretBox.NONCE_SIZE)
        encrypted = verification_crypto_box.encrypt(
            self.user_validator, verification_nonce
        )
        ...
        data = {
            ...
            "verification_nonce": nacl.encoding.HexEncoder.encode(
                verification_nonce
            ).decode(),
        }

        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + self.request_data.get("token"),
    ...
    def test_post_authentication_activate_token_token_with_outstanding_ga_challenge(
        self,
    ):
        """
    ...
    def test_post_authentication_activate_token_token_with_outstanding_duo_challenge(
        self,
    ):
        """
    ...
    def test_post_authentication_activate_token_token_with_outstanding_yubikey_challenge(
        self,
    ):
        """
    ...
    def test_post_authentication_activate_token_verification_decrypt_fail(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_400_BAD_REQUEST)
        self.assertEqual(
            response.data.get("non_field_errors"), ["VERIFICATION_CODE_INCORRECT"]
        )
    ...
    def test_post_authentication_activate_token_wrong_user_validator(self):
        ...
        verification_nonce = nacl.utils.random(nacl.secret.SecretBox.NONCE_SIZE)
        encrypted = verification_crypto_box.encrypt(
            "asdf".encode("utf-8"), verification_nonce
        )
        verification = encrypted[len(verification_nonce) :]
...
class UserDeleteTests(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        self.test_user_sauce = (
            "b4ce697723a93e3ba36e6da23c8728c2372069fdffc2d29c02f77cd14a106c45"
        )
        self.test_user_obj = models.User.objects.create(
            username=self.test_username,
            email=encrypt_with_db_secret(self.test_email),
    ...
    def test_delete_user(self):
        ...
        data = {"authkey": self.test_authkey}
