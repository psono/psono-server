...
class SecretCountsUtilsTest(TestCase):
    ...
    def setUp(self):
        ...
        self.user1 = User.objects.create(
            ...
            private_key_nonce="".join(
                random.choice(string.ascii_lowercase + string.digits) for _ in range(48)
            ),
    ...
    def test_secrets_in_user_datastores_only(self):
        ...
        # Assert results
        self.assertEqual(
            result[str(self.user1.id)], 2, "User 1 should have 2 secrets from datastore"
        )
        self.assertEqual(
            result[str(self.user2.id)], 1, "User 2 should have 1 secret from datastore"
        )
    ...
    def test_secrets_in_directly_accessible_shares(self):
        ...
        self.assertEqual(
            result[str(self.user1.id)],
            0,
            "User 1 should have 0 secrets (no rights to shares)",
        )
        self.assertEqual(
            result[str(self.user2.id)],
            0,
            "User 2 should have 0 secrets (no rights to shares)",
        )
        self.assertEqual(
            result[str(self.user3.id)],
            3,
            "User 3 should have 3 secrets from direct share access",
        )
    ...
    def test_secrets_in_nested_shares(self):
        ...
        self.assertEqual(
            result[str(self.user2.id)],
            2,
            "User 2 should have 2 secrets from nested share",
        )
    ...
    def test_secrets_through_group_membership(self):
        ...
        User_Group_Membership.objects.create(
            user_id=self.user3.id, group_id=group.id, accepted=True
        )
        ...
        self.assertEqual(
            result[str(self.user3.id)],
            2,
            "User 3 should have 2 secrets through group membership",
        )
    ...
    def test_mixed_access_patterns(self):
        ...
        self.assertEqual(
            result[str(self.user1.id)],
            1,
            "User 1 should have 1 secret from own datastore",
        )
        self.assertEqual(
            result[str(self.user2.id)],
            3,
            "User 2 should have 3 secrets (1 datastore + 2 share access)",
        )
        self.assertEqual(
            result[str(self.user3.id)],
            1,
            "User 3 should have 1 secret through group access",
        )
    ...
    def test_no_duplicate_secret_counting(self):
        ...
        self.assertEqual(
            result[str(self.user2.id)],
            1,
            "Secret should be counted only once despite multiple access paths",
        )
    ...
    def test_nonexistent_user(self):
        ...
        self.assertEqual(
            result[fake_user_id], 0, "Non-existent user should have 0 secrets"
        )
    ...
    def test_unaccepted_rights_ignored(self):
        ...
        self.assertEqual(
            result[str(self.user2.id)], 0, "Unaccepted share rights should be ignored"
        )
