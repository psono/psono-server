...
class CommandVerifyuseremailTestCase(TestCase):
    ...
    def test_verifyuseremail(self):
        """
        Tests to verify the email of a user
        """
        ...
        out = StringIO()
        call_command('verifyuseremail', stdout=out, *args, **opts)

        user = models.User.objects.get(username=self.test_username)

        self.assertTrue(user.is_email_active)

    def test_verifyuseremail_does_not_exist(self):
        """
        Tests to verify the email of a user that does not exist
        """
