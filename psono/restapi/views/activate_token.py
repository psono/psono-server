        ...
        if not settings.DISABLE_EMAIL_NEW_LOGIN:
            ...
            with translation.override(request.LANGUAGE_CODE):
                ...
                if request.user.zoneinfo:
                    login_datetime_timezone = login_datetime.astimezone(
                        request.user.zoneinfo
                    )
                    login_datetime_timezone_str = (
                        date_format(login_datetime_timezone, format="DATETIME_FORMAT")
                        + " "
                        + login_datetime_timezone.tzname()
                    )
                else:
                    login_datetime_timezone_str = (
                        date_format(login_datetime, format="DATETIME_FORMAT") + " UTC"
                    )

                subject = (
                    render_to_string(
                        "email/new_login_subject.txt",
                        {
                            ...
                            "webclient_url_with_wbr": "<wbr>".join(
                                splitAt(settings.WEB_CLIENT_URL, 40)
                            ),
