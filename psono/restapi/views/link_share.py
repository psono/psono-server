...
class LinkShareView(GenericAPIView):
    ...
    def get(self, request, link_share_id=None, *args, **kwargs):
        ...
        if not link_share_id:
            ...
            for link_share in (
                Link_Share.objects.filter(user=request.user)
                .exclude(valid_till__lt=timezone.now())
                .exclude(allowed_reads__lte=0)
            ):
                link_shares.append(
                    {
                        "id": link_share.id,
            ...
            return Response({"link_shares": link_shares}, status=status.HTTP_200_OK)
        else:
    ...
    def put(self, request, *args, **kwargs):
        ...
        serializer = CreateLinkShareSerializer(
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = UpdateLinkShareSerializer(
            data=request.data, context=self.get_serializer_context()
        )
