...
from ..models import Share, User_Share_Right
...
class ShareView(GenericAPIView):
    ...
    def get_shares(self, user):
        ...
        user_share_rights = (
            User_Share_Right.objects.select_related("creator")
            .filter(user=user)
        ...
        for user_share_right in user_share_rights:
            share = {
                ...
                "share_right_expiration_date": user_share_right.expiration_date.isoformat()
                if user_share_right.expiration_date
                else None,
                "share_right_create_user_id": user_share_right.creator.id
                if user_share_right.creator is not None
                else "",
                "share_right_create_user_username": user_share_right.creator.username
                if user_share_right.creator is not None
                else "",
                "share_right_create_user_public_key": user_share_right.creator.public_key
                if user_share_right.creator is not None
                else "",
            }
    ...
    def get(self, request, share_id=None, *args, **kwargs):
        ...
        if not share_id:
            ...
            serializer = ReadShareSerializer(
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not create_share_link(
            ...
            share.id,
            serializer.validated_data["parent_share_id"],
            serializer.validated_data["parent_datastore_id"],
        ):
            return Response(
                {"error": "DuplicateLinkID", "message": "Don't use a link id twice"},
