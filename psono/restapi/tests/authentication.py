...
class AuthenticationTests(APITestCaseExtended):
    ...
    def test_user_token_to_token_hash(self):
        token = "1234"
        self.assertEqual(
            TokenAuthentication.user_token_to_token_hash(token),
            sha512(token.encode()).hexdigest(),
        )
...
class AuthenticateTests(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        encrypted = self.secret_box.encrypt(
            decrypted_user_validator, verification_nonce
        )
    ...
    def test_authenticate_failure_corrupted_authorization_validator(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(
            response.data, {"detail": "Invalid token header. Not proper encrypted."}
        )

    @patch("restapi.authentication.settings", DEVICE_PROTECTION_DISABLED=False)
    def test_authenticate_device_protection_failure_request_device_fingerprint_missing(
        self, settings_fct
    ):
        ...
        self.assertEqual(
            response.data,
            {
                "detail": "Device Fingerprint Protection: request_device_fingerprint missing"
            },
        )

    @patch("restapi.authentication.settings", DEVICE_PROTECTION_DISABLED=False)
    def test_authenticate_device_protection_failure_request_device_fingerprint_missmatch(
        self, settings_fct
    ):
        ...
        encrypted = self.secret_box.encrypt(
            json.dumps({"request_device_fingerprint": "1234"}).encode("utf-8"),
            authorization_validator_nonce,
        )
        authorization_validator = encrypted[len(authorization_validator_nonce) :]
        authorization_validator_hex = nacl.encoding.HexEncoder.encode(
        ...
        self.assertEqual(
            response.data,
            {"detail": "Device Fingerprint Protection: device_fingerprint mismatch"},
        )

    @patch(
        "restapi.authentication.settings",
        DEVICE_PROTECTION_DISABLED=False,
        AUTO_PROLONGATION_TOKEN_TIME_VALID=0,
    )
    def test_authenticate_device_protection_success_legacy_variable(self, settings_fct):
        ...
        encrypted = self.secret_box.encrypt(
            json.dumps({"request_device_fingerprint": "123456"}).encode("utf-8"),
            authorization_validator_nonce,
        )
        authorization_validator = encrypted[len(authorization_validator_nonce) :]
        authorization_validator_hex = nacl.encoding.HexEncoder.encode(
    ...
    def test_authenticate_device_protection_success(self, settings_fct):
        ...
        encrypted = self.secret_box.encrypt(
            json.dumps({"request_device_session": "123456"}).encode("utf-8"),
            authorization_validator_nonce,
        )
        authorization_validator = encrypted[len(authorization_validator_nonce) :]
        authorization_validator_hex = nacl.encoding.HexEncoder.encode(
    ...
    def test_authenticate_replay_protection_failure_request_time_missing(
        self, settings_fct
    ):
        ...
        self.assertEqual(response.status_code, status.HTTP_401_UNAUTHORIZED)
        self.assertEqual(
            response.data, {"detail": "Replay Protection: request_time missing"}
        )
    ...
    def test_authenticate_replay_protection_failure_request_time_too_old(
        self, settings_fct
    ):
        ...
        encrypted = self.secret_box.encrypt(
            json.dumps({"request_time": "2000-01-01T10:00:00.000Z"}).encode("utf-8"),
            authorization_validator_nonce,
        )
        authorization_validator = encrypted[len(authorization_validator_nonce) :]
        authorization_validator_hex = nacl.encoding.HexEncoder.encode(
    ...
    def test_authenticate_replay_protection_success(self, settings_fct):
        ...
        encrypted = self.secret_box.encrypt(
            json.dumps({"request_time": timezone.now().isoformat()}).encode("utf-8"),
            authorization_validator_nonce,
        )
        authorization_validator = encrypted[len(authorization_validator_nonce) :]
        authorization_validator_hex = nacl.encoding.HexEncoder.encode(
