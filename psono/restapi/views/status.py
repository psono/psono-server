...
class StatusView(GenericAPIView):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if user_status is None:
            unaccepted_shares_count = (
                User_Share_Right.objects.filter(
                    user=request.user, accepted__isnull=True
                )
            ...
            unaccepted_groups_count = User_Group_Membership.objects.filter(
                user=request.user, accepted__isnull=True, private_key__isnull=False
            ).count()

            try:
                latest_security_report = SecurityReport.objects.filter(
                    user=request.user
                ).latest("create_date")
            ...
            if latest_security_report:
                last_security_report_created = (
                    latest_security_report.create_date.isoformat()
                )
            else:
