...
class FileserverAlive(APITestCaseExtended):
    ...
    def test_alive_initial(self):
        ...
        cluster_crypto_box = Box(
            PrivateKey(self.cluster_private_key_hex, encoder=nacl.encoding.HexEncoder),
        ...
        box = PrivateKey.generate()
        fileserver_private_key_hex = box.encode(
            encoder=nacl.encoding.HexEncoder
        ).decode()
        fileserver_public_key_hex = box.public_key.encode(
            encoder=nacl.encoding.HexEncoder
        ).decode()
        ...
        encrypted = cluster_crypto_box.encrypt(
            json.dumps(decrypted_fileserver_info).encode("utf-8"), nonce
        )
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + fileserver_id,
        ...
        self.assertEqual(
            Fileserver_Cluster_Members.objects.filter(key=token_hash).count(), 1
        )

        fileserver = Fileserver_Cluster_Members.objects.get(key=token_hash)
        self.assertEqual(
            Fileserver_Cluster_Member_Shard_Link.objects.filter(
                member_id=fileserver.id, shard_id=str(self.shard1.id)
            ).count(),
            1,
        )
