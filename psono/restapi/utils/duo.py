...
def duo_auth_preauth(integration_key: str, secret_key: str, host: str, user_id: str) -> dict:
    """
    Preauth call with the user id
    ...
    try:
        ...
        preauth = auth_api.preauth(user_id=user_id)
    except gaierror:
        return {
            'error': 'Host incorrect: Could not be found'
        }
...
def duo_auth_authstatus(integration_key: str, secret_key: str, host: str, txid: str) -> dict:
    """
    Auth status call with the txid
    ...
    try:
        ...
        auth_status = auth_api.auth_status(txid=txid)
    except gaierror:
        return {
            'error': 'Host incorrect: Could not be found'
        }
