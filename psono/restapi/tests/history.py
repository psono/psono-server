...
class ReadHistory(APITestCaseExtended):
    ...
    def test_put(self):
        ...
        url = reverse(
            "history",
            kwargs={"secret_history_id": str(self.test_secret_history_obj.id)},
        )
    ...
    def test_read_history_of_secret_success(self):
        ...
        self.assertEqual(
            response.data.get("create_date"),
            self.test_secret_history_obj.create_date.isoformat(),
        )
        self.assertEqual(
            response.data.get("write_date"),
            self.test_secret_history_obj.write_date.isoformat(),
        )
        self.assertEqual(
            response.data.get("data"), self.test_secret_history_obj.data.decode()
        )
        self.assertEqual(
            response.data.get("data_nonce"), self.test_secret_history_obj.data_nonce
        )
        self.assertEqual(response.data.get("type"), self.test_secret_history_obj.type)
        self.assertEqual(
            response.data.get("callback_url"), self.test_secret_history_obj.callback_url
        )
        self.assertEqual(
            response.data.get("callback_user"),
            self.test_secret_history_obj.callback_user,
        )
        self.assertEqual(
            response.data.get("callback_pass"),
            self.test_secret_history_obj.callback_pass,
        )
    ...
    def test_read_history_of_secret_failure_does_not_exist(self):
        ...
        url = reverse(
            "history",
            kwargs={"secret_history_id": "53d78f4f-0512-4cbf-9ecb-c75f3fe11c8c"},
        )
