...
class SecurityReportView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = CreateSecurityReportSerializer(
        ...
        for entry in entries:
            if "master_password" in entry and entry["master_password"]:
                master_password_duplicate = (
                    entry["duplicate"] if "duplicate" in entry else None
                )
                master_password_breached = (
                    entry["breached"] if "breached" in entry else None
                )
                master_password_length = (
                    entry["password_length"] if "password_length" in entry else None
                )
                master_password_variation_count = (
                    entry["variation_count"] if "variation_count" in entry else None
                )
        ...
        security_report = SecurityReport.objects.create(
            user=request.user,
            recovery_code_exists=Recovery_Code.objects.filter(
                user=request.user
            ).exists(),
        ...
        SecurityReportEntry.objects.bulk_create(
            [
                SecurityReportEntry(
                    ...
                    create_age=timedelta(days=entry["create_age"])
                    if "create_age" in entry
                    else None,
                    write_age=timedelta(days=entry["write_age"])
                    if "write_age" in entry
                    else None,
                    ...
                    if "password_length" in entry
                    else None,
                    variation_count=entry["variation_count"]
                    if "variation_count" in entry
                    else None,
                )
