...
def translate_text(text, lang):
    response = client.chat.completions.create(
        ...
        messages=[
            ...
            {
                "role": "user",
                "content": f"Translate the following text to language code '{lang}' and provide only the translated text with no additional commentary or explanations: {text}",
            },
        ],
    )
...
def translate_language(lang):
    ...
    if (
        ...
        or "status" not in content["response"]
        or content["response"]["status"] != "success"
    ):
        print("Error: upload_language " + lang)
        print(r.text)
        exit(1)
...
def get_languages():
    ...
    r = requests.post(
        "https://api.poeditor.com/v2/languages/list", data=data, timeout=20.0
    )
    if not r.ok:
