...
def link_shard(
    cluster_id: str, shard_id: str, permission: str, fix_link_id=None
) -> dict:
    ...
    if (
        fix_link_id is None
        and Fileserver_Cluster_Shard_Link.objects.filter(
            cluster_id=cluster_id, shard_id=shard_id
        ).count()
        > 0
    ):
        return {"error": "A link between this shard and cluster already exists"}
