...
class MetadataShareView(GenericAPIView):
    ...
    def get_serializer_class(self):
        if self.request.method == "GET":
            return ReadMetadataShareSerializer
        return Serializer
    ...
    def get(self, request, *args, **kwargs):
        ...
        serializer = ReadMetadataShareSerializer(
        ...
        secrets = []
        for secret_link in (
            Secret_Link.objects.select_related("secret")
            .only("secret_id", "secret__write_date")
            .filter(parent_share_id=share.id)
        ):
            secrets.append(
                {
        ...
        shares = []
        for share_tree_entry in (
            Share_Tree.objects.select_related("share")
            .only("share_id", "share__write_date")
            .filter(parent_share_id=share.id)
        ):
            shares.append(
                {
