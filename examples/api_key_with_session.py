...
def api_update_secret(token, session_secret_key, secret_id, data, data_nonce):
    ...
    data = json.dumps({"secret_id": secret_id, "data": data, "data_nonce": data_nonce})
...
def main():
    ...
    for datastore in content["datastores"]:
        ...
        datastore_content_encrypted = decrypt_symmetric(
    ...
    if datastore_secret is None:
        print(
            "No website password found in datastore. This demo script expects a website password in the root of the datastore"
        )
        return

    # 7. Read a secret
    secret_read_result = api_read_secret(
        token, session_secret_key, datastore_secret["secret_id"]
    )
    secret_content = json.loads(
        decrypt_symmetric(
            secret_read_result["data"],
            secret_read_result["data_nonce"],
            datastore_secret["secret_key"],
        )
    )
    ...
    encrypted_secret = encrypt_symmetric(
        json.dumps(secret_content), datastore_secret["secret_key"]
    )
    api_update_secret(
        token,
        session_secret_key,
        datastore_secret["secret_id"],
