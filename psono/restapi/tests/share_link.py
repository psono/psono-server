...
class ShareTests(APITestCaseExtended):
    ...
    def test_share_tree_on_insert(self):
        ...
        self.assertNotEqual(
            response.data.get("share_id", False),
            False,
            "Share id does not exist in share PUT answer",
        )
        self.assertIsUUIDString(
            str(response.data.get("share_id", "")), "Share id is no valid UUID"
        )
        ...
        self.assertEqual(
            share_trees.count(),
            1,
            "Exactly 1 share tree object should be created, but we got: "
            + str(share_trees.count()),
        )
        ...
        self.assertEqual(
            share_trees.count(),
            2,
            "Exactly 2 share tree objects should be created, but we got: "
            + str(share_trees.count()),
        )
        ...
        for t in share_trees:
            ...
            self.assertEqual(
                str(t.share_id),
                new_child_share_id,
                "Share should be the last created share",
            )
        ...
        self.assertEqual(
            share_trees.count(),
            3,
            "Exactly 3 share tree objects should be created, but we got: "
            + str(share_trees.count()),
        )
...
class ShareTreeModificationTests(APITestCaseExtended):
    def setUp(self):
        ...
        self.test_private_key_nonce3 = (
            "4298a9ab3d9d5d8643dfd4445adc30301b56aab650497fb8"
        )
    ...
    def test_link_of_share_to_other_parent_share_without_rights_for_the_parent_share(
        self,
    ):
        """
    ...
    def test_link_of_share_to_other_parent_share(self):
        ...
        self.assertEqual(
            share_trees.count(),
            expected_share_tree_count,
            "Exactly "
            + str(expected_share_tree_count)
            + " share tree objects should be created, but we got: "
            + str(share_trees.count()),
        )
        ...
        share_trees = models.Share_Tree.objects.filter(
            path__match="*." + str(link_id).replace("-", "") + ".*"
        )
        ...
        self.assertEqual(
            share_trees[0].parent_share_id,
            request_data["parent_share_id"],
            "Parent share is incorrect",
        )
        self.assertEqual(
            share_trees[0].parent_datastore_id, None, "Datastore is incorrect"
        )

        expected_path = (
            str(self.initial_data5["link_id"]).replace("-", "")
            + "."
            + str(link_id).replace("-", "")
        )
        self.assertEqual(share_trees[0].path, expected_path, "Path is incorrect")
        ...
        self.assertEqual(
            share_trees[0].share_id,
            self.response8.data.get("share_id"),
            "Share is incorrect",
        )
        self.assertEqual(
            share_trees[0].parent_datastore_id, None, "Datastore is incorrect"
    ...
    def test_link_of_share_in_datastore(self):
        ...
        self.assertEqual(
            share_trees[0].parent_share_id, None, "Parent share is incorrect"
        )
        self.assertEqual(
            share_trees[0].parent_datastore_id,
            request_data["parent_datastore_id"],
    ...
    def test_delete_share_that_does_not_exist(self):
        ...
        request_data = {"link_id": "b48cbd50-1aba-4389-90ce-3bed32c831e3"}
    ...
    def test_delete_share_that_exists(self):
        ...
        self.assertEqual(
            share_trees.count(),
            expected_share_tree_count_after,
            "Exactly "
            + str(expected_share_tree_count_after)
            + " share tree objects should be created, but we got: "
            + str(share_trees.count()),
        )
        ...
        request_data = {"link_id": str(link_id)}
        ...
        self.assertEqual(
            share_trees.count(),
            expected_share_tree_count_before,
            "Exactly "
            + str(expected_share_tree_count_before)
            + " share tree objects should be created, but we got: "
            + str(share_trees.count()),
        )
    ...
    def test_delete_share_with_no_write_rights_on_parent_share(self):
        ...
        models.User_Share_Right.objects.filter(
            share_id=self.response5.data.get("share_id"), user=self.test_user_obj
        ).update(write=False)
    ...
    def test_delete_share_with_no_write_rights_on_parent_datastore(self):
        ...
        models.Data_Store.objects.filter(pk=self.test_datastore1_obj.id).update(
            user=self.test_user3_obj
        )
    ...
    def test_move_share_to_new_parent_share_with_no_write_rights_on_new_parent(self):
        ...
        models.User_Share_Right.objects.filter(
            share_id=self.response6.data.get("share_id"), user=self.test_user_obj
        ).update(write=False)
    ...
    def test_move_share_to_new_parent_share_while_no_grant_permissions_on_share(self):
        ...
        # change share_rights to no write
        models.User_Share_Right.objects.filter(
            share_id=self.response8.data.get("share_id"), user=self.test_user_obj
        ).update(grant=False)
    ...
    def test_move_from_not_own_datastore_to_own_datastore(self):
        ...
        models.User_Share_Right.objects.filter(
            share_id=self.responseB3.data.get("share_id"), user=self.test_user2_obj
        ).update(user=self.test_user_obj)
