...
class GroupView(GenericAPIView):
    ...
    def get_groups(self, request):
        ...
        memberships = (
            User_Group_Membership.objects.select_related("group")
            .filter(user=request.user)
            .exclude(accepted=False)
            .exclude(private_key__isnull=True)
            .distinct()
        )
        ...
        for membership in memberships:
            ...
            if membership.accepted is None:
                details["user_id"] = (
                    membership.creator.id if membership.creator is not None else ""
                )
                details["user_username"] = (
                    membership.creator.username
                    if membership.creator is not None
                    else ""
                )
    ...
    def get_group_details(self, request, membership):
        ...
        if membership.accepted is None:
            ...
            response["user_username"] = (
                membership.creator.username if membership.creator is not None else ""
            )
    ...
    def get(self, request, group_id=None, *args, **kwargs):
        ...
        if not group_id:
            return Response(
                {"groups": self.get_groups(request)}, status=status.HTTP_200_OK
            )
        else:
            # Returns the specified group if the user has any rights for it
            try:
                membership = User_Group_Membership.objects.get(
                    user=request.user, group_id=group_id, private_key__isnull=False
                )
            except User_Group_Membership.DoesNotExist:
                return Response(
                    {"message": "NO_PERMISSION_OR_NOT_EXIST", "resource_id": group_id},
                    status=status.HTTP_400_BAD_REQUEST,
                )

            return Response(
                self.get_group_details(request, membership), status=status.HTTP_200_OK
            )
