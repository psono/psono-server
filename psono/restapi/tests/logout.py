...
class LogoutTests(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        self.test_user_sauce = (
            "24350a638726c0073ec43c8c84ac110bfc2c45e7a430a257f768837f1470c9c7"
        )
        ...
        tok = models.Token.objects.filter(
            key=TokenAuthentication.user_token_to_token_hash(self.test_token)
        ).get()
        tok.active = True
        ...
        self.tok2 = models.Token.objects.filter(
            key=TokenAuthentication.user_token_to_token_hash(self.test_token2)
        ).get()
        ...
        # encrypt authorization validator with session key
        secret_box = nacl.secret.SecretBox(
            self.tok2.secret_key, encoder=nacl.encoding.HexEncoder
        )
        authorization_validator_nonce = nacl.utils.random(
            nacl.secret.SecretBox.NONCE_SIZE
        )
    ...
    def test_logout_false_token(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + self.test_token + "hackIT",
            HTTP_AUTHORIZATION_VALIDATOR=self.authorization_validator1,
        )
        response = self.client.post(url)
        self.assertEqual(
            response.status_code, status.HTTP_401_UNAUTHORIZED, "Any login is accepted"
        )
    ...
    def test_logout_other_token(self):
        ...
        self.client.credentials(
            HTTP_AUTHORIZATION="Token " + self.test_token2,
            HTTP_AUTHORIZATION_VALIDATOR=self.authorization_validator2,
        )
        response = self.client.post(url)
        self.assertEqual(
            response.status_code,
    ...
    def test_logout_other_token_that_does_not_exist(self):
        ...
        updated_data = {"session_id": "5ae48987-29c2-4c07-b50e-4ee35556d63e"}
