...
class LinkShareAccess(APITestCaseExtended):
    ...
    def test_put_allowed_reads_being_decremented(self):
        ...
        self.assertTrue(
            models.Link_Share.objects.filter(
                pk=self.link_share.id, allowed_reads=1
            ).exists()
        )
        ...
        # ... and then delete it
        self.assertFalse(
            models.Link_Share.objects.filter(pk=self.link_share.id).exists()
        )
    ...
    def test_put_link_share_no_operations_anymore(self):
        ...
        self.link_share.allow_write = True
        self.link_share.allowed_reads = (
            0  # This is the counter that handles writes when writes are enabled
        )
        self.link_share.save()
    ...
    def test_post_success_with_secret(self):
        ...
        data = {"link_share_id": str(self.link_share.id)}
    ...
    def test_post_success_with_secret_in_share_with_allow_write_false(self):
        ...
        data = {"link_share_id": str(self.link_share_in_share.id)}
