...
class EmergencyLoginView(GenericAPIView):
    ...
    def put(self, request, *args, **kwargs):
        ...
        serializer = ActivateEmergencyLoginSerializer(
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = EmergencyLoginSerializer(
            data=request.data, context=self.get_serializer_context()
        )
        ...
        if not emergency_code.activation_date and user.email:
            ...
            with translation.override(user.language):
                subject = (
                    render_to_string(
                        "email/emergency_code_armed_subject.txt",
                        {
                ...
                msg_plain = render_to_string(
                    "email/emergency_code_armed.txt",
                    {
                        "emergency_code_description": emergency_code.description,
                        "emergency_code_activation_delay": emergency_code.activation_delay,
                        "activation_link": emergency_code_link,
                ...
                msg_html = render_to_string(
                    "email/emergency_code_armed.html",
                    {
                        "emergency_code_description": emergency_code.description,
                        "emergency_code_activation_delay": emergency_code.activation_delay,
                        "activation_link": emergency_code_link,
        ...
        remaining_wait_time = (
            emergency_code.activation_date
            + timedelta(seconds=emergency_code.activation_delay)
            - timezone.now()
        )
        if (
            emergency_code.activation_delay > 0
            and remaining_wait_time.total_seconds() > 0
        ):
            return Response(
                {
                    "remaining_wait_time": int(
        ...
        emergency_code.verifier = verifier_box.encode(
