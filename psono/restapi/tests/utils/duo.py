...
class TestDuoAuthEnrollStatusUtils(TestCase):
    ...
    def test_duo_auth_check_success(self):
        result = duo_auth_enroll_status(
            "integration_key", "secret_key", "host", "user_id", "activation_code"
        )
...
class TestDuoAuthAuthUtils(TestCase):
    ...
    def mock_invalid_host(
        self, username, factor, device, pushinfo, passcode, async_txn
    ):
        raise gaierror

    def mock_invalid_cert(
        self, username, factor, device, pushinfo, passcode, async_txn
    ):
        raise SSLError

    def mock_invalid_integration_key(
        self, username, factor, device, pushinfo, passcode, async_txn
    ):
        raise RuntimeError("Invalid integration key")

    def mock_invalid_secret_key(
        self, username, factor, device, pushinfo, passcode, async_txn
    ):
        raise RuntimeError("Invalid signature")

    def mock_other_api_error(
        self, username, factor, device, pushinfo, passcode, async_txn
    ):
        raise RuntimeError("Other API Error")
    ...
    def test_duo_auth_check_success(self):
        result = duo_auth_auth(
            "integration_key", "secret_key", "host", "username", "factor", "device"
        )
