...
class CommandEnableuserTestCase(TestCase):
    ...
    def test_enableuser(self):
        """
        Tests to enable a user
        """
        ...
        out = StringIO()
        call_command('enableuser', stdout=out, *args, **opts)

        user = models.User.objects.get(username=self.test_username)

        self.assertTrue(user.is_active)

    def test_enableuser_does_not_exist(self):
        """
        Tests to enable a user that does not exist
        """
