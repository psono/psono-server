...
class UpdateLinkShareSerializer(serializers.Serializer):
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        try:
            link_share = Link_Share.objects.get(
                id=link_share_id, user=self.context["request"].user
            )
        ...
        if (
            link_share.secret_id is not None
            and allow_write
            and not user_has_rights_on_secret(
                self.context["request"].user.id, link_share.secret_id, write=True
            )
        ):
            msg = "NO_PERMISSION_OR_NOT_EXIST"
            raise exceptions.ValidationError(msg)
