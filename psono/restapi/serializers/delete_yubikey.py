...
class DeleteYubikeySerializer(serializers.Serializer):
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        try:
            yubikey_otp = Yubikey_OTP.objects.get(
                pk=yubikey_otp_id, user=self.context["request"].user
            )
        except Yubikey_OTP.DoesNotExist:
            msg = "NO_PERMISSION_OR_NOT_EXIST"
            raise exceptions.ValidationError(msg)

        yubikey_otp_count = Yubikey_OTP.objects.filter(
            user=self.context["request"].user, active=True
        ).count()
