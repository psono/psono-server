...
from ..app_settings import CreateShareRightSerializer
from ..app_settings import UpdateShareRightSerializer
from ..app_settings import DeleteShareRightSerializer
...
class ShareRightView(GenericAPIView):
    ...
    def get(self, request, user_share_right_id=None, *args, **kwargs):
        ...
        if not user_share_right_id:
            ...
            user_share_rights = (
                User_Share_Right.objects.filter(user=request.user, accepted=True)
                .filter(
            ...
            for share_right in user_share_rights:
                if share_right.share_id not in share_right_index:
                    ...
                    share_right_index[share_right.share_id]["read"] = (
                        share_right_index[share_right.share_id]["read"]
                        or share_right.read
                    )
                    share_right_index[share_right.share_id]["write"] = (
                        share_right_index[share_right.share_id]["write"]
                        or share_right.write
                    )
                    share_right_index[share_right.share_id]["grant"] = (
                        share_right_index[share_right.share_id]["grant"]
                        or share_right.grant
                    )
            ...
            response = {"share_rights": share_right_response}
            ...
            try:
                ...
                if (
                    share_right.creator_id != request.user.id
                    and share_right.user_id != request.user.id
                ):
                    return Response(
                        {
                            "message": "NO_PERMISSION_OR_NOT_EXIST",
                            "resource_id": user_share_right_id,
                        },
                        status=status.HTTP_403_FORBIDDEN,
                    )
                if (
                    share_right.expiration_date
                    and share_right.expiration_date <= timezone.now()
                ):
                    return Response(
                        {
                            "message": "NO_PERMISSION_OR_NOT_EXIST",
            ...
            response = {
                ...
                "grant": share_right.grant,
                "share_id": share_right.share_id,
                "expiration_date": share_right.expiration_date.isoformat()
                if share_right.expiration_date
                else None,
            }
    ...
    def put(self, request, *args, **kwargs):
        ...
        serializer = CreateShareRightSerializer(
        ...
        if serializer.validated_data.get("user_id", False):
            ...
            if (
                get_all_inherited_rights(
                    serializer.validated_data["user_id"],
                    serializer.validated_data["share_id"],
                )
                is not None
            ):
                accepted = True
            ...
            if settings.CACHE_ENABLE:
                cache_key = "psono_user_status_" + str(
                    serializer.validated_data["user"].id
                )
                cache.delete(cache_key)

            if (
                not settings.DISABLE_EMAIL_NEW_SHARE_CREATED
                and serializer.validated_data["user"].email
            ):
                # send email
                if settings.WEB_CLIENT_URL:
                    pending_share_link = (
                        settings.WEB_CLIENT_URL + "/index.html#!/share/pendingshares"
                    )
                else:
                ...
                with translation.override(serializer.validated_data["user"].language):
                    subject = (
                        render_to_string(
                            "email/new_share_created_subject.txt",
                    ...
                    msg_plain = render_to_string(
                        "email/new_share_created.txt",
                        {"pending_share_link": pending_share_link},
                    )
                    msg_html = render_to_string(
                        "email/new_share_created.html",
                        {"pending_share_link": pending_share_link},
                    )
            ...
            if not settings.DISABLE_EMAIL_NEW_GROUP_SHARE_CREATED:
                users = (
                    User.objects.filter(
                        group_memberships__group=serializer.validated_data["group"]
                    )
                    .exclude(id=request.user.id)
                    .values("language", "email")
                )
                emails_by_language = defaultdict(list)
                for user in users:
                    if not user["email"]:
                        continue
                    emails_by_language[user["language"]].append(
                ...
                for language, email_addresses in emails_by_language.items():
                    with translation.override(language):
                        subject = (
                            render_to_string(
                                "email/new_group_share_created_subject.txt",
                                {"pending_share_link": pending_share_link},
                            )
                            .replace("\n", " ")
                            .replace("\r", "")
                        ...
                        msg_plain = render_to_string(
                            "email/new_group_share_created.txt",
                            {"pending_share_link": pending_share_link},
                        )
                        msg_html = render_to_string(
                            "email/new_group_share_created.html",
                            {"pending_share_link": pending_share_link},
                        )
        ...
        return Response(
            {"share_right_id": share_right.id}, status=status.HTTP_201_CREATED
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        # it does not yet exist, so lets create it
        serializer = UpdateShareRightSerializer(
            data=request.data, context=self.get_serializer_context()
        )
        ...
        if "expiration_date" in serializer.validated_data:
            share_right_obj.expiration_date = serializer.validated_data.get(
                "expiration_date"
            )
        share_right_obj.save()

        return Response(
            {"share_right_id": str(share_right_obj.id)}, status=status.HTTP_200_OK
        )
    ...
    def delete(self, request, *args, **kwargs):
        ...
        # it does not yet exist, so lets create it
        serializer = DeleteShareRightSerializer(
            data=request.data, context=self.get_serializer_context()
        )
