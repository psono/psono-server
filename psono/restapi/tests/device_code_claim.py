...
class ClaimDeviceCodeTest(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        self.test_authkey = nacl.encoding.HexEncoder.encode(
            nacl.utils.random(settings.AUTH_KEY_LENGTH_BYTES)
        ).decode()

        self.user1_private_key = PrivateKey.generate()
        self.user1_public_key = self.user1_private_key.public_key
        self.test_public_key_hex = self.user1_public_key.encode(
            encoder=nacl.encoding.HexEncoder
        ).decode()
        self.test_private_key_hex = self.user1_private_key.encode(
            encoder=nacl.encoding.HexEncoder
        ).decode()

        self.test_secret_key_hex = nacl.encoding.HexEncoder.encode(
            nacl.utils.random(settings.USER_SECRET_KEY_LENGTH_BYTES)
        ).decode()
        self.test_secret_key_nonce_hex = nacl.encoding.HexEncoder.encode(
            nacl.utils.random(settings.NONCE_LENGTH_BYTES)
        ).decode()
        self.test_private_key_nonce_hex = nacl.encoding.HexEncoder.encode(
            nacl.utils.random(settings.NONCE_LENGTH_BYTES)
        ).decode()
        ...
        self.other_authkey = nacl.encoding.HexEncoder.encode(
            nacl.utils.random(settings.AUTH_KEY_LENGTH_BYTES)
        ).decode()

        self.user2_private_key = PrivateKey.generate()
        self.user2_public_key = self.user2_private_key.public_key
        self.other_public_key_hex = self.user2_public_key.encode(
            encoder=nacl.encoding.HexEncoder
        ).decode()
        self.other_private_key_hex = self.user2_private_key.encode(
            encoder=nacl.encoding.HexEncoder
        ).decode()

        self.other_secret_key_hex = nacl.encoding.HexEncoder.encode(
            nacl.utils.random(settings.USER_SECRET_KEY_LENGTH_BYTES)
        ).decode()
        self.other_secret_key_nonce_hex = nacl.encoding.HexEncoder.encode(
            nacl.utils.random(settings.NONCE_LENGTH_BYTES)
        ).decode()
        self.other_private_key_nonce_hex = nacl.encoding.HexEncoder.encode(
            nacl.utils.random(settings.NONCE_LENGTH_BYTES)
        ).decode()
        ...
        self.dc_server_public_key_hex = self.dc_server_private_key.public_key.encode(
            encoder=nacl.encoding.HexEncoder
        ).decode()

        self.dc_user_private_key_for_pending = (
            PrivateKey.generate()
        )  # Simulating device's key
        self.dc_user_public_key_for_pending_hex = (
            self.dc_user_private_key_for_pending.public_key.encode(
                encoder=nacl.encoding.HexEncoder
            ).decode()
        )
        ...
        self.pending_device_code = models.DeviceCode.objects.create(
            ...
            server_private_key=self.dc_server_private_key.encode(
                encoder=nacl.encoding.HexEncoder
            ).decode(),
        ...
        self.active_device_code = models.DeviceCode.objects.create(
            ...
            encrypted_credentials_nonce=nacl.encoding.HexEncoder.encode(
                nacl.utils.random(settings.NONCE_LENGTH_BYTES)
            ).decode()[:64],
            valid_till=timezone.now() + timedelta(minutes=default_validity_minutes),
        )
        ...
        server_pub_key_for_box = PublicKey(
            self.pending_device_code.server_public_key, encoder=nacl.encoding.HexEncoder
        )
        ...
        self.valid_nonce_hex = nacl.encoding.HexEncoder.encode(
            self.valid_nonce_bytes
        ).decode()

        encrypted_data_bytes = box.encrypt(
            self.plain_credentials, self.valid_nonce_bytes
        )
        self.valid_encrypted_credentials_hex = nacl.encoding.HexEncoder.encode(
            encrypted_data_bytes.ciphertext
        ).decode()
    ...
    def test_claim_device_code_success(self):
        ...
        self.assertEqual(
            self.pending_device_code.state, models.DeviceCode.DeviceCodeState.CLAIMED
        )
        ...
        self.assertEqual(
            self.pending_device_code.encrypted_credentials_nonce, self.valid_nonce_hex
        )

        # Check response data (serializer output)
        self.assertEqual(
            response.data["state"], models.DeviceCode.DeviceCodeState.CLAIMED.value
        )  # Serializer returns value
        self.assertEqual(response.data["user"], self.test_user_obj.id)
        self.assertEqual(
            response.data["server_public_key"],
            self.pending_device_code.server_public_key,
        )
        self.assertEqual(
            response.data["user_public_key"], self.pending_device_code.user_public_key
        )
        self.assertEqual(
            response.data["encrypted_credentials_nonce"], self.valid_nonce_hex
        )
        ...
        device_pub_key_for_box = PublicKey(
            self.pending_device_code.user_public_key, encoder=nacl.encoding.HexEncoder
        )
        decrypt_box_server_perspective = Box(dc_server_priv_key, device_pub_key_for_box)

        try:
            decrypted_by_server = decrypt_box_server_perspective.decrypt(
                nacl.encoding.HexEncoder.decode(response_encrypted_hex),
                nacl.encoding.HexEncoder.decode(self.valid_nonce_hex),
            )
            self.assertEqual(
                decrypted_by_server,
                self.plain_credentials,
                "Credentials in response not decryptable by server or content mismatch.",
            )
    ...
    def test_claim_device_code_already_active(self):
        ...
        """
        url = reverse(
            "device_code_claim", kwargs={"device_code": str(self.active_device_code.id)}
        )
        payload = {
            "encrypted_credentials_input": self.valid_encrypted_credentials_hex,
            "encrypted_credentials_nonce": self.valid_nonce_hex,
        ...
        self.assertEqual(
            self.active_device_code.state, models.DeviceCode.DeviceCodeState.CLAIMED
        )
    ...
    def test_claim_device_code_already_expired_past_valid_till(self):
        ...
        url = reverse(
            "device_code_claim",
            kwargs={"device_code": str(self.expired_device_code.id)},
        )
        payload = {
            "encrypted_credentials_input": self.valid_encrypted_credentials_hex,
            "encrypted_credentials_nonce": self.valid_nonce_hex,
        ...
        self.assertEqual(
            self.expired_device_code.state, models.DeviceCode.DeviceCodeState.PENDING
        )
        ...
        self.assertEqual(
            self.expired_device_code.state, models.DeviceCode.DeviceCodeState.EXPIRED
        )
    ...
    def test_claim_device_code_becomes_expired_on_attempt(self):
        ...
        self.assertEqual(
            self.pending_device_code_will_expire.state,
            models.DeviceCode.DeviceCodeState.PENDING,
        )
        self.assertTrue(
            self.pending_device_code_will_expire.valid_till > timezone.now()
        )

        url = reverse(
            "device_code_claim",
            kwargs={"device_code": str(self.pending_device_code_will_expire.id)},
        )
        payload = {
            "encrypted_credentials_input": self.valid_encrypted_credentials_hex,
            "encrypted_credentials_nonce": self.valid_nonce_hex,
        ...
        future_time = self.pending_device_code_will_expire.valid_till + timedelta(
            seconds=1
        )
        ...
        self.assertEqual(
            self.pending_device_code_will_expire.state,
            models.DeviceCode.DeviceCodeState.EXPIRED,
        )
    ...
    def test_claim_device_code_user_mismatch(self):
        ...
        url = reverse(
            "device_code_claim",
            kwargs={"device_code": str(self.pending_dc_assigned_other_user.id)},
        )
        payload = {
        ...
        self.assertEqual(
            self.pending_dc_assigned_other_user.state,
            models.DeviceCode.DeviceCodeState.PENDING,
        )
        ...
        for field, value, err_string, err_code in test_cases:
            with self.subTest(field=field, value=value, error_string=err_string):
                ...
                self.assertEqual(
                    ...
                    f"Failed for {field}={value}. Response: {response.data}",
                )
                self.assertIn(
                    field,
                    response.data,
                    f"Field {field} not in error response for value {value}. Response: {response.data}",
                )
    ...
    def test_http_method_not_allowed(self):
        ...
        self.assertEqual(
            response_get.data,
            {"detail": "GET method not supported. Use PUT to claim a device code."},
        )
        ...
        self.assertEqual(
            response_post.data,
            {"detail": "POST method not supported. Use PUT to claim a device code."},
        )
        ...
        self.assertEqual(
            ...
            {"detail": "DELETE method not supported. Use PUT to claim a device code."},
        )
        ...
        self.assertIn(
            "method not supported", response_patch.data.get("detail", "").lower()
        )
