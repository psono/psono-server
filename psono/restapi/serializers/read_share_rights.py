...
class ReadShareRightsSerializer(serializers.Serializer):
    def validate(self, attrs: dict) -> dict:
        share_id = (
            self.context["request"].parser_context["kwargs"].get("share_id", False)
        )
        ...
        own_share_rights = calculate_user_rights_on_share(
            self.context["request"].user.id, share_id
        )

        if not any(
            [
                own_share_rights["read"],
                own_share_rights["write"],
                own_share_rights["grant"],
            ]
        ):
            msg = "NO_PERMISSION_OR_NOT_EXIST"
            raise exceptions.ValidationError(msg)
