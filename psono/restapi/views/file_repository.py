...
class FileRepositoryView(GenericAPIView):
        ...
        if not file_repository_id:
            ...
            for file_repository_right in File_Repository.objects.filter(
                ...
                file_repository_index[str(file_repository_right.id)] = {
                    ...
                    "file_repository_right_id": str(
                        file_repository_right.file_repository_right_id
                    ),
                }
            ...
            for file_repository_right in File_Repository.objects.raw(
                """SELECT fr.id, fr.title, fr.type, fr.active, gfrr.read, gfrr.write, gfrr.grant
                ...
                if str(file_repository_right.id) in file_repository_index:
                    file_repository_index[str(file_repository_right.id)]["read"] = (
                        file_repository_index[str(file_repository_right.id)]["read"]
                        or file_repository_right.read
                    )
                    file_repository_index[str(file_repository_right.id)]["write"] = (
                        file_repository_index[str(file_repository_right.id)]["write"]
                        or file_repository_right.write
                    )
                    file_repository_index[str(file_repository_right.id)]["grant"] = (
                        file_repository_index[str(file_repository_right.id)]["grant"]
                        or file_repository_right.grant
                    )
                    continue
            ...
            return Response(
                {"file_repositories": list(file_repository_index.values())},
            ...
            try:
                file_repository = File_Repository.objects.get(pk=file_repository_id)
            except File_Repository.DoesNotExist:
                return Response(
                    {
                        "message": "NO_PERMISSION_OR_NOT_EXIST",
                        "resource_id": file_repository_id,
                    },
            ...
            if rights["grant"]:
                for file_repository_right in (
                    File_Repository_Right.objects.filter(
                        file_repository_id=file_repository_id
                    )
                    .select_related("user")
                    .only(
            ...
            if rights["grant"]:
                for file_repository_right in (
                    Group_File_Repository_Right.objects.filter(
                        file_repository_id=file_repository_id
                    )
                    .select_related("group")
                    .only("id", "group__id", "group__name", "read", "write", "grant")
                    .all()
                ):
    ...
    def put(self, request, *args, **kwargs):
        ...
        serializer = CreateFileRepositorySerializer(
        ...
        file_repository = File_Repository.objects.create(
            ...
            data=encrypt_with_db_secret(
                json.dumps(serializer.validated_data.get("data"))
            ).encode(),
            active=True,
        )
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = UpdateFileRepositorySerializer(
            data=request.data, context=self.get_serializer_context()
        )
        ...
        file_repository.data = encrypt_with_db_secret(
            json.dumps(serializer.validated_data.get("data"))
        ).encode()
    ...
    def delete(self, request, *args, **kwargs):
        ...
        serializer = DeleteFileRepositorySerializer(
            data=request.data, context=self.get_serializer_context()
        )
