...
class CommandFSShardUnlinkTestCase(TestCase):
    ...
    def test_unlink_shard(self):
        """
        Tests to unlink a shard
        """

        args = [str(self.link.id)]
        opts = {}

        out = StringIO()
        call_command('fsshardunlink', stdout=out, *args, **opts)

        self.assertFalse(models.Fileserver_Cluster_Shard_Link.objects.exists())
