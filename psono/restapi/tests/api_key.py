...
class ReadApiKeyTest(APITestCaseExtended):
    ...
    def test_read_api_keys_success(self):
        ...
        self.assertEqual(
            api_key.get("restrict_to_secrets"),
    ...
    def test_read_api_key_success(self):
        ...
        self.assertEqual(
            response.data.get("public_key"), self.test_api_key_obj.public_key
        )
        self.assertEqual(
            response.data.get("secret_key"), self.test_api_key_obj.secret_key
        )
        self.assertEqual(
            response.data.get("secret_key_nonce"),
            self.test_api_key_obj.secret_key_nonce,
        )
        self.assertEqual(
            response.data.get("private_key"), self.test_api_key_obj.private_key
        )
        self.assertEqual(
            response.data.get("private_key_nonce"),
            self.test_api_key_obj.private_key_nonce,
        )
        ...
        self.assertEqual(
            response.data.get("restrict_to_secrets"),
            self.test_api_key_obj.restrict_to_secrets,
        )
        self.assertEqual(
    ...
    def test_read_api_key_failure_not_exist(self):
        ...
        url = reverse(
            "api_key", kwargs={"api_key_id": "212dbbe0-7d1c-4136-b514-6c9d374d579c"}
        )
