...
class UserShareRightsTest(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        self.test_share2_obj = models.Share.objects.create(
            user_id=self.test_user_obj.id, data=b"my-data_2", data_nonce="12345_2"
        )
    ...
    def test_post_share_rights(self):
        ...
        url = reverse(
            "share_rights", kwargs={"share_id": "cebcd1c4-baf6-441f-b97f-796ba6c95848"}
        )
    ...
    def test_list_shares_without_valid_share_id(self):
        ...
        self.assertEqual(
            response.data.get("non_field_errors"), ["NO_PERMISSION_OR_NOT_EXIST"]
        )
    ...
    def test_list_shares(self):
        ...
        self.assertEqual(response.status_code, status.HTTP_200_OK)
        self.assertNotEqual(
            response.data.get("id", False), False, "id does not exist in answer"
        )
        self.assertNotEqual(
            response.data.get("own_share_rights", False),
            False,
            "own_share_rights does not exist in answer",
        )
        self.assertNotEqual(
            response.data.get("user_share_rights", False),
            False,
