...
class RecoveryCodeView(GenericAPIView):
    ...
    def get_serializer_class(self):
        if self.request.method == "POST":
            return CreateRecoverycodeSerializer
        return Serializer
    ...
    def post(self, request, *args, **kwargs):
        ...
        serializer = CreateRecoverycodeSerializer(
        ...
        recovery_code = Recovery_Code.objects.create(
            user=request.user,
            recovery_authkey=make_password(
                str(serializer.validated_data["recovery_authkey"])
            ),
        ...
        return Response(
            {"recovery_code_id": recovery_code.id}, status=status.HTTP_200_OK
        )
