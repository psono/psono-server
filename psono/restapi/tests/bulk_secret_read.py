...
class BulkReadSecretTest(APITestCaseExtended):
    ...
    def test_read_secret_in_datastore(self):
        ...
        self.assertIn(
            str(self.test_secret_obj1.id), [s["id"] for s in response_data["secrets"]]
        )
        self.assertIn(
    ...
    def test_read_secret_in_shares(self):
        ...
        self.assertIn(
            str(self.test_secret_obj4.id), [s["id"] for s in response_data["secrets"]]
        )
        self.assertIn(
            str(self.test_secret_obj5.id), [s["id"] for s in response_data["secrets"]]
        )
    ...
    def test_db_queries(self):
        ...
        data = {
            "secret_ids": [
                ...
                str(self.test_secret_obj3.id),
                str(
                    self.test_secret_obj4.id
                ),  # This secret resides in a share and should cause 2 extra DB query
            ],
        }
        ...
        data = {
            "secret_ids": [
                ...
                str(
                    self.test_secret_obj4.id
                ),  # This secret resides in a share and should cause 1 extra DB query
