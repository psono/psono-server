...
class RegisterView(GenericAPIView):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if not settings.ALLOW_REGISTRATION or not settings.WEB_CLIENT_URL:
            return Response(
                {"custom": ["REGISTRATION_HAS_BEEN_DISABLED"]},
                status=status.HTTP_400_BAD_REQUEST,
            )

        if (
            settings.ENFORCE_MATCHING_USERNAME_AND_EMAIL
            and self.request.data.get("username", "").lower()
            != self.request.data.get("email", "").lower()
        ):
            return Response(
                {"custom": ["REGISTRATION_FAILED_USERNAME_AND_EMAIL_HAVE_TO_MATCH"]},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        try:
            ...
            return Response(
                {"custom": ["REGISTRATION_FAILED_USERNAME_ALREADY_EXISTS"]},
                status=status.HTTP_400_BAD_REQUEST,
            )
        ...
        with translation.override(request.LANGUAGE_CODE):
            subject = (
                render_to_string(
                    "email/registration_successful_subject.txt",
                    {
                        "email": self.request.data.get("email", ""),
                        "username": self.request.data.get("username", ""),
                        "activation_code": activation_code,
                        "activation_link": activation_link,
                        "activation_link_with_wbr": "<wbr>".join(
                            splitAt(activation_link, 40)
                        ),
            ...
            msg_plain = render_to_string(
                "email/registration_successful.txt",
                {
                    "email": self.request.data.get("email", ""),
                    "username": self.request.data.get("username", ""),
                    "activation_code": activation_code,
            ...
            msg_html = render_to_string(
                "email/registration_successful.html",
                {
                    "email": self.request.data.get("email", ""),
                    "username": self.request.data.get("username", ""),
                    "activation_code": activation_code,
        ...
        try:
            ...
            user.delete()
            return Response(
                {"non_field_errors": ["REGISTRATION_EMAIL_DELIVERY_FAILED"]},
                status=status.HTTP_400_BAD_REQUEST,
            )

        return Response(
            {"success": "REGISTRATION_SUCCESSFUL"}, status=status.HTTP_201_CREATED
        )
