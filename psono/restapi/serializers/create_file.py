...
class CreateFileSerializer(serializers.Serializer):
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        if shard_id is not None:
            ...
            try:
                shard = Fileserver_Shard.objects.only("id").get(
                    pk=shard_id, active=True
                )
        ...
        if file_repository_id is not None:
            ...
            try:
                file_repository = File_Repository.objects.only(
                    "id", "type", "data"
                ).get(pk=file_repository_id, active=True)
        ...
        has_share_or_datastore = (
            parent_share_id is not None or parent_datastore_id is not None
        )
        ...
        if has_secret_parent:
            if not user_has_rights_on_secret(
                self.context["request"].user.id, parent_secret_id, write=True
            ):
                msg = "NO_PERMISSION_OR_NOT_EXIST"
                raise exceptions.ValidationError(msg)
        ...
        if has_link:
            ...
            if parent_datastore_id is not None:
                parent_datastore = get_datastore(
                    parent_datastore_id, self.context["request"].user
                )
        ...
        if shard_id:
            cluster_member_shard_link_objs = (
                ...
                .only(
                    "write", "ip_write_blacklist", "ip_write_whitelist", "member__write"
                )
            )
