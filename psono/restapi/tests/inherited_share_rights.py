...
class UserShareRightsWithInheritedRightTest(APITestCaseExtended):
    ...
    def test_first_level_inherited_rights(self):
        ...
        url = reverse(
            "share", kwargs={"share_id": str(self.response11.data["share_id"])}
        )
    ...
    def test_first_level_inherited_rights_expired(self):
        # setup: create rights for share1 for user2 and then expire them
        url = reverse("share_right")
        ...
        initial_data = {
            ...
            "grant": True,
            "user_id": str(self.test_user2_obj.id),
            "expiration_date": (timezone.now() + timedelta(minutes=5)).isoformat(),
        }
        ...
        # lets approve it and force it to be expired
        models.User_Share_Right.objects.filter(
            pk=response.data["share_right_id"]
        ).update(
            accepted=True,
        ...
        # lets query for share11, expected failure because inherited right is expired
        url = reverse(
            "share", kwargs={"share_id": str(self.response11.data["share_id"])}
        )
