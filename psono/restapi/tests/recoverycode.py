...
class RecoveryCodeTests(APITestCaseExtended):
    def setUp(self):
        ...
        self.test_user_sauce = (
            "ef37b3192178b9a97b551572314388058c14a4dabdbf63d022bcba9951809b6d"
        )
        ...
        models.Google_Authenticator.objects.create(
            user=self.test_user_obj, title="My TItle", secret="1234"
        )
        ...
        models.Yubikey_OTP.objects.create(
            user=self.test_user_obj, title="My Sweet Title", yubikey_id="1234"
        )
...
class PasswordTests(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        self.verifier_public_key = (
            "7a372bb1558b0d42eaac3e238e633efd997f6496c62302bdb56c3a729a7ce41c"
        )

        self.user_public_key = (
            "618ccedc6edc9ee8110f8a75e7bb24238759fe43f638ad41d399dae7043f9d1d"
        )
        ...
        self.test_user_sauce = (
            "1f3f3c0f4c8a52fb0d83144bb4e7aaf04d552d43ec7a60792654ef664af17dad"
        )
        self.test_user_obj = models.User.objects.create(
            email=self.test_email,
            email_bcrypt=self.test_email_bcrypt,
        ...
        self.test_user_sauce2 = (
            "fbcd7106abf5ef076af9a1ab59e98ff5f4f81f524ede6d7155500e059b25b8b0"
        )
        ...
        self.test_authkey3 = binascii.hexlify(
            os.urandom(settings.AUTH_KEY_LENGTH_BYTES)
        ).decode()
        self.test_private_key3 = binascii.hexlify(
            os.urandom(settings.USER_PRIVATE_KEY_LENGTH_BYTES)
        ).decode()
        self.test_private_key_nonce3 = binascii.hexlify(
            os.urandom(settings.NONCE_LENGTH_BYTES)
        ).decode()
        self.test_secret_key3 = binascii.hexlify(
            os.urandom(settings.USER_SECRET_KEY_LENGTH_BYTES)
        ).decode()
        self.test_secret_key_nonce3 = binascii.hexlify(
            os.urandom(settings.NONCE_LENGTH_BYTES)
        ).decode()
        self.test_user_sauce3 = (
            "de6096562c48b5f58aaabfa9dfab3a59930daf57aa50f53a4d80d8205a91ba17"
        )
    ...
    def test_put_password_with_missing_authkey(self):
        ...
        crypto_box = Box(
            PrivateKey(self.user_private_key, encoder=nacl.encoding.HexEncoder),
    ...
    def test_put_password(self):
        ...
        self.assertFalse(
            models.Google_Authenticator.objects.filter(user=self.test_user_obj).exists()
        )
        self.assertFalse(
            models.Yubikey_OTP.objects.filter(user=self.test_user_obj).exists()
        )
        self.assertFalse(models.Duo.objects.filter(user=self.test_user_obj).exists())

        db_recovery_code = models.Recovery_Code.objects.get(
            pk=self.test_recovery_code_obj.id
        )
    ...
    def test_post_password(self):
        ...
        self.assertTrue("recovery_data_nonce" in response.data)
        self.assertEqual(
            response.data["recovery_data_nonce"], self.test_recovery_data_nonce
        )
        ...
        self.assertEqual(
            response.data["verifier_time_valid"], settings.RECOVERY_VERIFIER_TIME_VALID
        )
        self.assertTrue("recovery_sauce" in response.data)
