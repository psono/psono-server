...
class UserGetSecretTest(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        self.test_private_key_nonce3 = (
            "4228a9ab3d8d5d8643dfd4445adc30301b565ab650497fb9"
        )
    ...
    def test_with_not_existing_secret(self):
        ...
        url = reverse(
            "secret", kwargs={"secret_id": "cf84fbd5-c606-4d5b-aa96-88c68a06cde4"}
        )
    ...
    def test_read_secret_read_count(self):
        ...
        self.assertTrue(
            models.Secret.objects.filter(
                pk=self.test_secret_obj.id, read_count=0
            ).exists()
        )
        ...
        self.assertTrue(
            models.Secret.objects.filter(
                pk=self.test_secret_obj.id, read_count=1
            ).exists()
        )
        ...
        self.assertTrue(
            models.Secret.objects.filter(
                pk=self.test_secret_obj.id, read_count=2
            ).exists()
        )
...
class UserUpdateSecretTest(APITestCaseExtended):
    ...
    def test_with_not_existing_secret(self):
        ...
        data = {"secret_id": "cf84fbd5-c606-4d5b-aa96-88c68a06cde4"}
    ...
    def test_success(self):
        ...
        self.assertEqual(
            updated_secret.data.decode(), data["data"], "data in secret was not updated"
        )

        self.assertEqual(
            updated_secret.data_nonce,
            data["data_nonce"],
    ...
    @patch("requests.post", side_effect=mock_request_post)
    def test_success_with_callback_url_and_prefix_match_by_widlcard(
    ...
    @patch("requests.post", side_effect=mock_request_post)
    def test_with_callback_url_with_callback_prefix_not_matching(
        self, mock_request_post
    ):
        """
    ...
    @override_settings(DISABLE_CALLBACKS=False)
    @patch("requests.post", side_effect=mock_request_post)
    def test_success_with_callback_url_user_and_pass_that_is_not_proper_encrypted(
        self, mock_request_post
    ):
        """
