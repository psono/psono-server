...
def create_folder_if_not_exist(folder_name, parent_folder):
    ...
    Always creates a new folder, even if one with the same name exists
...
def is_website_address(address):
    ...
    common_tlds = ['.com', '.org', '.net', '.edu', '.gov', '.io', '.co', '.uk', '.de', '.fr', '.au', '.ca', '.jp', '.cn']
...
def create_website_password_secret(token, session_secret_key, name, username, address, password, folder, datastore_id):
    """
...
def create_application_password_secret(token, session_secret_key, name, username, address, password, folder, datastore_id):
    """
    ...
    content = {
        ...
        'application_password_notes': address if address else '',  # Store address in notes for application passwords
    }
...
def create_secret(token, session_secret_key, name, username, address, password, folder, datastore_id):
    """
    ...
    if is_website_address(address):
        create_website_password_secret(token, session_secret_key, name, username, address, password, folder, datastore_id)
    else:
        create_application_password_secret(token, session_secret_key, name, username, address, password, folder, datastore_id)
...
def main():
    if len(sys.argv) < 2:
        ...
        print("  safe_filter    - (Optional) Only import entries for this specific safe")
        print("\nCSV Format (semicolon-separated):")
        print('"safe";"Path";"name of object";"login/username";"address";"password"')
        print('\nExample:')
        print('"myid";"Root";"root account";"root";"192.168.1.1";"secretpass"')
        print('"myid";"Root\\Subfolder";"google";"user@gmail.com";"https://google.com";"pass123"')
        print('\nUsage examples:')
        print('  python import_csv_to_datastore.py passwords.csv           # Import all safes')
        print('  python import_csv_to_datastore.py passwords.csv myid      # Import only "myid" safe')
        sys.exit(1)
    ...
    print(f"Entries grouped into {len(grouped_entries)} safe(s): {', '.join(grouped_entries.keys())}")
    ...
    if decrypted_sever_login_info['api_key_restrict_to_secrets']:
        print("Error: API key is restricted. It should be unrestricted for this operation.")
        return
    ...
    for datastore in content['datastores']:
        ...
        datastore_content = json.loads(decrypt_symmetric(
    ...
    if datastore_id is None:
        print("Error: No password datastore found. Please create one with the webclient first.")
        return
    ...
    for safe_name, safe_entries in grouped_entries.items():
        ...
        # Create the top-level safe folder
        safe_folder = create_folder_if_not_exist(safe_name, datastore_content)

        # Process each entry in this safe
        for entry in safe_entries:
            print(f"  Creating: {entry['path']}/{entry['name']}")

            # Get or create the folder path within the safe
            target_folder = get_or_create_folder_path(entry['path'], safe_folder)
    ...
    if 'folders' in datastore_content:
