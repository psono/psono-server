...
class UserRightsAccept(APITestCaseExtended):
    ...
    def setUp(self):
        ...
        self.test_private_key_nonce3 = (
            "4398a9ab3d9d5d8643dfd4445adc30301b565ab650497fb8"
        )
        ...
        self.test_parent_share2_obj = models.Share.objects.create(
            user_id=self.test_user2_obj.id, data=b"my-data", data_nonce="12345"
        )
        ...
        self.test_parent_share3_obj = models.Share.objects.create(
            user_id=self.test_user3_obj.id, data=b"my-data", data_nonce="12345"
        )
    ...
    def test_accept_share_right_with_parent_share(self):
        ...
        self.assertEqual(
            response.data.get("share_data_nonce", False),
            self.test_share1_obj.data_nonce,
        )
        self.assertEqual(
            response.data.get("share_data", False), self.test_share1_obj.data.decode()
        )
        self.assertIsUUIDString(
            str(response.data.get("share_id", "")), "share id is no valid UUID"
        )

        saved_user_share_right = models.User_Share_Right.objects.get(
            pk=str(test_share_right1_obj.id)
        )

        self.assertEqual(
            saved_user_share_right.accepted, True, "Accepted flag should be true"
        )
        self.assertEqual(
            saved_user_share_right.key, initial_data["key"], "Key should be new key"
        )
        self.assertEqual(
            saved_user_share_right.key_nonce,
            initial_data["key_nonce"],
            "Key nonce should be new key",
        )
        self.assertEqual(
            saved_user_share_right.key_type,
            "symmetric",
            "Key type should now be symmetric",
        )
