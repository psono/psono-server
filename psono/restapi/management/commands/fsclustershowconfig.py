...
def show_cluster_config(cluster_id: str) -> dict:
    ...
    if cluster.links.count() == 0:
        return {
            "error": '    Shard: Not found for this cluster. You can create one with "fsshardcreate TITLE \'Some description\'" and then link it to this cluster with "fsshardlink '
            + str(cluster.id)
            + ' SHARD_ID"'
        }
    ...
    print(
        "SECRET_KEY: "
        + repr(("".join([secrets.choice(uni) for i in range(50)])).replace("'", '"'))
    )
    ...
    for links in cluster.links.all():
        shards.append(
            ...
            + str(links.write)
            + ", delete: "
            + str(links.delete_capability)
            + ", allow_link_shares: "
            + str(links.allow_link_shares)
