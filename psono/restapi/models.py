...
class User(models.Model):
    ...
    email_bcrypt = models.CharField(
        "bcrypt of email address", db_index=True, max_length=60
    )
    ...
    private_key_nonce = models.CharField(
        "private key nonce", max_length=64, unique=True
    )
    ...
    is_staff = models.BooleanField(
        "Has managemnet capabilities",
        default=False,
        help_text="Designates whether this user has management capabilities or not.",
    )

    authentication = models.CharField(
        "Authentication method", max_length=16, default="AUTHKEY"
    )
    duo_enabled = models.BooleanField(
        "Duo 2FA enabled", default=False, help_text="True once duo 2fa is enabled"
    )
    google_authenticator_enabled = models.BooleanField(
        "GA 2FA enabled", default=False, help_text="True once ga 2fa is enabled"
    )
    yubikey_otp_enabled = models.BooleanField(
        "Yubikey OTP 2FA enabled",
        default=False,
    ...
    ivalt_enabled = models.BooleanField(
        "iValt 2FA enabled", default=False, help_text="True once ivalt 2fa is enabled"
    )
    require_password_change = models.BooleanField(
        "require password change", default=False
    )
    ...
    hashing_algorithm = models.CharField(
        "hashing algorithm",
        max_length=32,
        default=DEFAULT_HASHING_ALGORITHM,
        choices=HASHING_ALGORITHMS,
    )
    hashing_parameters = models.JSONField(
        "hashing parameters", default=default_hashing_parameters
    )
    zoneinfo = TimeZoneField(null=True)
    country = CountryField(null=True)

    credit = models.DecimalField(
        max_digits=24, decimal_places=16, default=Decimal(str(0))
    )

    language = models.CharField("language", max_length=16, default="en")
    external_id = models.CharField(
        "external id", max_length=64, null=True, db_index=True
    )
    ...
    def save(self, *args, **kwargs):
        ...
        try:
            ...
            secret_key_nonce_changed = (
                self.secret_key_nonce != stored_user.secret_key_nonce
            )
            private_key_changed = self.private_key != stored_user.private_key
            private_key_nonce_changed = (
                self.private_key_nonce != stored_user.private_key_nonce
            )
            email_changed = self.email != stored_user.email
            email_bcrypt_changed = self.email_bcrypt != stored_user.email_bcrypt
            hashing_algorithm_changed = (
                self.hashing_algorithm != stored_user.hashing_algorithm
            )
            hashing_parameters_changed = (
            ...
            if (
                ...
                or secret_key_changed
                or secret_key_nonce_changed
    ...
    def any_2fa_active(self):
        return (
            self.yubikey_otp_enabled
            or self.google_authenticator_enabled
            or self.duo_enabled
            or self.webauthn_enabled
            or self.ivalt_enabled
        )
...
class Old_Credential(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="old_credentials"
    )
    authkey = models.CharField("auth key", max_length=128, null=True)
    public_key = models.CharField("public key", max_length=256)
    private_key = models.CharField("private key", max_length=256)
...
class API_Key(models.Model):
    ...
    user_private_key_nonce = models.CharField(
        "user private key nonce", max_length=64, unique=True
    )
    user_secret_key = models.CharField("user secret key", max_length=256)
    user_secret_key_nonce = models.CharField(
        "user secret key nonce", max_length=64, unique=True
    )

    verify_key = models.CharField("verify key", max_length=64)

    read = models.BooleanField("Read right", default=True, help_text="Allows reading")
    write = models.BooleanField(
        "Write right", default=False, help_text="Allows writing / updating"
    )

    restrict_to_secrets = models.BooleanField(
        "Restrict to specific secrets",
        default=False,
        help_text="Allows access to only spcific secrets",
    )
    allow_insecure_access = models.BooleanField(
...
class Google_Authenticator(models.Model):
    ...
    write_date = models.DateTimeField(auto_now=True)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="google_authenticator"
    )
    title = models.CharField("title", max_length=256)
...
class Webauthn(models.Model):
    ...
    credential_id = models.TextField(
        "The credential id as passed from the frontend", default=""
    )
    credential_public_key = models.TextField(
        "The credential public key as passed from the frontend", default=""
    )
...
class Recovery_Code(models.Model):
    ...
    recovery_data_nonce = models.CharField(
        "recovery data nonce", max_length=64, unique=True
    )
...
class Emergency_Code(models.Model):
    ...
    write_date = models.DateTimeField(auto_now=True)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="emergency_code"
    )
    emergency_authkey = models.CharField("emergency auth key", max_length=128)
    emergency_data = models.BinaryField()
    emergency_data_nonce = models.CharField(
        "emergency data nonce", max_length=64, unique=True
    )
    verifier = models.CharField("last verifier", max_length=256)
    verifier_issue_date = models.DateTimeField(null=True, blank=True)
    ...
    activation_date = models.DateTimeField(
        "Date this emergency code becomes active", null=True, blank=True
    )
...
class Secret(models.Model):
    ...
    write_date = models.DateTimeField(auto_now=True)
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="secrets"
    )
    data = models.BinaryField()
...
class API_Key_Secret(models.Model):
    ...
    api_key = models.ForeignKey(
        API_Key, on_delete=models.CASCADE, related_name="api_key_secrets"
    )
    secret = models.ForeignKey(
        Secret, on_delete=models.CASCADE, related_name="api_key_secrets"
    )
    secret_key = models.CharField("secret key", max_length=256)
    secret_key_nonce = models.CharField("secret key nonce", max_length=64)
    title = models.CharField("title", max_length=256)
...
class Secret_History(models.Model):
    ...
    write_date = models.DateTimeField(auto_now=True)
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="secret_history"
    )
...
class Group(models.Model):
    ...
    public_key = models.CharField("public key", max_length=256)
    forced_membership = models.BooleanField(
        "Forced Membership",
        default=False,
...
class Secret_Link(models.Model):
    ...
    parent_datastore = models.ForeignKey(
        Data_Store,
        on_delete=models.CASCADE,
        related_name="parent_links",
        null=True,
...
class Group_Share_Right(models.Model):
    ...
    group = models.ForeignKey(
        Group,
        on_delete=models.CASCADE,
        related_name="group_share_rights",
        help_text="The group who will receive this share right",
    )
    creator = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        related_name="own_group_share_rights",
        help_text="The user who created this share right",
        null=True,
    )
...
class User_Group_Membership(models.Model):
    ...
    secret_key_type = models.CharField(
        "Key type",
        default="asymmetric",
    ...
    private_key_type = models.CharField(
        "Private Key type",
        default="asymmetric",
    ...
    share_admin = models.BooleanField(
        "Share admin",
        default=True,
...
class User_Share_Right(models.Model):
    ...
    user = models.ForeignKey(
        User,
        on_delete=models.CASCADE,
        related_name="foreign_user_share_rights",
        help_text="The user who will receive this share right",
    )
    creator = models.ForeignKey(
        User,
        on_delete=models.SET_NULL,
        related_name="own_user_share_rights",
        help_text="The user who created this share right",
        null=True,
    )
    key = models.CharField(
    ...
    key_type = models.CharField(
        "Key type",
        default="asymmetric",
        help_text='Key type, either "symmetric", or "asymmetric"',
        max_length=16,
    )
    title = models.CharField(
        "Title",
    ...
    share = models.ForeignKey(
        Share,
        on_delete=models.CASCADE,
        related_name="user_share_rights",
        help_text="The share that this share right grants permissions to",
    )
    read = models.BooleanField(
        "Read right",
...
class Share_Tree(models.Model):
    ...
    parent_share = models.ForeignKey(
        Share,
        on_delete=models.CASCADE,
        related_name="parent_share_trees",
        null=True,
        help_text="The share, where this link ends and gets its permissions from",
    )
...
class Fileserver_Cluster(models.Model):
    ...
    file_size_limit = models.BigIntegerField(
        "File size limit", help_text="File size limit in bytes"
    )
    auth_public_key = models.CharField(
        "public key",
        max_length=256,
        help_text="Public key given to fileservers of this cluster to authenticate against the server",
    )
    auth_private_key = models.CharField(
        "private key",
        max_length=256,
...
class Fileserver_Cluster_Shard_Link(models.Model):
    ...
    read = models.BooleanField(
        "Read", default=True, help_text="Weather this shard accepts reads"
    )
    write = models.BooleanField(
        "Write", default=True, help_text="Weather this shard accepts writes"
    )
    allow_link_shares = models.BooleanField(
        "Allow link shares",
        default=True,
        help_text="Allows anonymous access with link shares",
    )
    delete_capability = models.BooleanField(
        "Delete", default=True, help_text="Weather this connection accepts deletes"
    )
...
class Fileserver_Cluster_Members(models.Model):
    ...
    fileserver_cluster = models.ForeignKey(
        Fileserver_Cluster,
        on_delete=models.CASCADE,
    ...
    read = models.BooleanField(
        "Read", default=True, help_text="Weather this server accepts reads"
    )
    write = models.BooleanField(
        "Write", default=True, help_text="Weather this server accepts writes"
    )
    allow_link_shares = models.BooleanField(
        "Allow link shares",
        default=True,
    ...
    delete_capability = models.BooleanField(
        "Delete", default=True, help_text="Weather this server accepts deletes"
    )
...
class File_Repository_Right(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="file_repository_right"
    )
    file_repository = models.ForeignKey(
        File_Repository, on_delete=models.CASCADE, related_name="file_repository_right"
    )
...
class Group_File_Repository_Right(models.Model):
    ...
    group = models.ForeignKey(
        Group, on_delete=models.CASCADE, related_name="file_repository_right"
    )
    file_repository = models.ForeignKey(
        File_Repository,
        on_delete=models.CASCADE,
...
class Fileserver_Cluster_Member_Shard_Link(models.Model):
    ...
    delete_capability = models.BooleanField(
        "Delete", default=True, help_text="Weather this shard accepts delete jobs"
    )
    ip_read_whitelist = models.CharField(
        "IP read whitelist",
        max_length=2048,
        help_text="IP Whitelist for read operations",
        null=True,
    )
    ip_write_whitelist = models.CharField(
        "IP write whitelist",
        max_length=2048,
        help_text="IP Whitelist for write operations",
        null=True,
    )
    ip_read_blacklist = models.CharField(
        "IP read blacklist",
        max_length=2048,
        help_text="IP Blacklist for read operations",
        null=True,
    )
    ip_write_blacklist = models.CharField(
        "IP write blacklist",
        max_length=2048,
        help_text="IP Blacklist for write operations",
        null=True,
    )
...
class File(models.Model):
    ...
    write_date = models.DateTimeField(auto_now=True)
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="file"
    )
    shard = models.ForeignKey(
        Fileserver_Shard, on_delete=models.CASCADE, null=True, related_name="file"
    )
    file_repository = models.ForeignKey(
        File_Repository, on_delete=models.CASCADE, null=True, related_name="file"
    )
    secret = models.ForeignKey(
        "Secret",
        on_delete=models.SET_NULL,
    ...
    chunk_count = models.IntegerField("Chunk Count", help_text="The amount of chunks")
                ...
                for chunk in self.file_chunk.all():
                    try:
                        if file_repository_type == "gcp_cloud_storage":
                            gcs_delete(
                                data["gcp_cloud_storage_bucket"],
                                data["gcp_cloud_storage_json_key"],
                                chunk.hash_checksum,
                            )
                            ...
                            backblaze_delete(
                                data["backblaze_bucket"],
                                data["backblaze_region"],
                                data["backblaze_access_key_id"],
                                data["backblaze_secret_access_key"],
                                chunk.hash_checksum,
                            )
                        elif (
                            file_repository_type == "other_s3"
                            and is_allowed_other_s3_endpoint_url(
                                data["other_s3_endpoint_url"]
                            )
                        ):
                            s3_delete(
                                data["other_s3_bucket"],
                                data["other_s3_region"],
...
class File_Chunk(models.Model):
    ...
    write_date = models.DateTimeField(auto_now=True)
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="file_chunk"
    )
    hash_checksum = models.CharField(max_length=128, unique=True)
    position = models.IntegerField(
        "Position", help_text="The position of the chunk in the file"
    )
...
class File_Transfer(models.Model):
    ...
    user = models.ForeignKey(
        User, on_delete=models.SET_NULL, null=True, related_name="file_transfer"
    )
    file = models.ForeignKey(
        File, on_delete=models.SET_NULL, null=True, related_name="file_transfer"
    )
    shard = models.ForeignKey(
        Fileserver_Shard,
        on_delete=models.SET_NULL,
        null=True,
        related_name="file_transfer",
    )
    file_repository = models.ForeignKey(
        File_Repository,
        on_delete=models.SET_NULL,
    ...
    size_transferred = models.BigIntegerField(
        "Transferred Size",
        help_text="The amount in bytes that have been transferred (including encryption overhead)",
    )
    chunk_count = models.IntegerField("Chunk Count", help_text="The amount of chunks")
    chunk_count_transferred = models.IntegerField(
        "Chunk Count Transfered", help_text="The amount of chunks already transfered"
    )
...
class Link_Share(models.Model):
    ...
    secret = models.ForeignKey(
        Secret, on_delete=models.CASCADE, related_name="link_shares", null=True
    )
    file = models.ForeignKey(
        File, on_delete=models.CASCADE, related_name="link_shares", null=True
    )
    public_title = models.CharField("Public Title", max_length=256)
    allow_write = models.BooleanField(
        "Allow write", default=False, help_text="Do we allow people to update a secret"
    )
    allowed_reads = models.IntegerField(
        "Allowed reads or writes",
        blank=True,
        null=True,
...
class SecurityReport(models.Model):
    ...
    write_date = models.DateTimeField(auto_now=True)
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="security_reports"
    )
    recovery_code_exists = models.BooleanField(
        "Recovery Code exists", default=False, help_text="Did a recovery code exist"
    )
    two_factor_exists = models.BooleanField(
        "Two factor exists",
        default=False,
        help_text="Did the user configure two factor exist",
    )
    website_password_count = models.PositiveIntegerField(
        "Count of passwords", default=0
    )
    breached_password_count = models.PositiveIntegerField(
        "Count of breached passwords", default=0
    )
    duplicate_password_count = models.PositiveIntegerField(
        "Count of password duplicates", default=0
    )
    check_haveibeenpwned = models.BooleanField(
        "Checked HaveIBeenPwned",
        default=False,
        help_text="Did the user check his passwords against have i been pwened",
    )

    master_password_breached = models.PositiveIntegerField(
        "Master password breached",
        default=None,
        help_text="Has the master password been breached",
        null=True,
    )
    master_password_duplicate = models.BooleanField(
        "Master password duplicate",
        default=False,
        help_text="Has the master password been used somewhere else",
        null=True,
    )
    master_password_length = models.PositiveIntegerField(
        "Master password length", null=True
    )
    master_password_variation_count = models.PositiveIntegerField(
        "Master password variation count",
        null=True,
        help_text="The count of variations (uppercase, lowercase, numbers, special chars",
    )
...
class SecurityReportEntry(models.Model):
    ...
    security_report = models.ForeignKey(
        SecurityReport, on_delete=models.CASCADE, related_name="security_report_entries"
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="security_report_entries"
    )
    name = models.TextField(null=True)
    type = models.TextField(null=True)
    create_age = models.DurationField(
        "Create Age", help_text="The time in days since its last update", null=True
    )
    write_age = models.DurationField(
        "Write Age", help_text="The time in days since its creation", null=True
    )
    ...
    variation_count = models.PositiveIntegerField(
        "Variation count",
        null=True,
        help_text="The count of variations (uppercase, lowercase, numbers, special chars",
    )
...
class DeviceCode(models.Model):
    ...
    state = models.CharField(
        max_length=16,
        null=False,
        choices=DeviceCodeState.choices,
        default=DeviceCodeState.PENDING,
    )
    user = models.ForeignKey(
        User, on_delete=models.CASCADE, related_name="device_codes", null=True
    )
...
class Token(models.Model):
    ...
    api_key = models.ForeignKey(
        API_Key, on_delete=models.CASCADE, null=True, related_name="tokens"
    )
    ...
    def _generate(self):
        ...
        self.user_validator = nacl.encoding.HexEncoder.encode(
            nacl.utils.random(nacl.secret.SecretBox.KEY_SIZE)
        ).decode()
...
def user_post_save_receiver(sender, **kwargs):
    if settings.CACHE_ENABLE:
        pk = str(kwargs["instance"].pk)
        cache.set(
            "psono_user_" + pk, kwargs["instance"], kwargs["instance"].get_cache_time()
        )
