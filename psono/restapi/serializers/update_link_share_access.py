...
class UpdateLinkShareAccessSerializer(serializers.Serializer):
    ...
    def validate(self, attrs: dict) -> dict:
        ...
        try:
            link_share = Link_Share.objects.select_related(
                "secret", "file", "user"
            ).get(id=link_share_id)
        ...
        if link_share.passphrase is not None and not check_password(
        ...
        if link_share.secret_id and not user_has_rights_on_secret(
            link_share.user_id, link_share.secret_id, read=True, write=write_required
        ):
            msg = "NO_PERMISSION_OR_NOT_EXIST"
            raise exceptions.ValidationError(msg)
        ...
        if link_share.file_id:
            if not user_has_rights_on_file(
                link_share.user_id, link_share.file, read=True
            ):
                msg = "NO_PERMISSION_OR_NOT_EXIST"
                raise exceptions.ValidationError(msg)
            ...
            if (
                link_share.file.delete_date
                and link_share.file.delete_date < timezone.now()
            ):
                msg = "NO_PERMISSION_OR_NOT_EXIST"
                raise exceptions.ValidationError(msg)

            if link_share.file.shard_id:
                cluster_member_shard_link_objs = (
                    Fileserver_Cluster_Member_Shard_Link.objects.select_related(
                        "member", "shard"
                    )
                    .filter(
                        ...
                        member__allow_link_shares=True,
                        shard=link_share.file.shard,
                    )
                    .only(
                        "read",
                        "ip_read_blacklist",
                ...
                if settings.SHARD_CREDIT_COSTS_DOWNLOAD > 0:
                    credit = (
                        settings.SHARD_CREDIT_COSTS_DOWNLOAD
                        * link_share.file.size
                        / 1024
                        / 1024
                        / 1024
                    )
